...
SELECT products.*, users.id, users.name FROM products INNER JOIN users ON products.userid = users.id;
SELECT users.*, address.name AS name, address.phone AS phone, address.address, address.city, address.postal_code FROM users INNER JOIN address ON users.email = address.email_user WHERE email = 'dikaaja3972@gmail.com';
SELECT address.*, users.name AS username FROM address INNER JOIN users ON users.email = address.email_user WHERE address.email_user = 'dikaaja3972@gmail.com';
SELECT transactions.*, bags.id_seller, bags.id_product FROM transactions INNER JOIN bags ON bags.id = transactions.id_bags WHERE email =
...
SELECT transactions.*, address.* FROM transactions INNER JOIN address ON transactions.id_bags;

SELECT bags.quantity, users.name AS seller_name, products.photo, products.name, products.brand, products.price FROM bags INNER JOIN users ON users.email = bags.id_seller INNER JOIN products ON products.id = bags.id_product WHERE id_customer = id;
...
SELECT product.*, size.name AS size_item, category.name AS category_item FROM product INNER JOIN size ON product.size = size.id INNER JOIN category ON product.category = category.id;

SELECT product.*, size.name AS size_item, category.name AS category_item FROM product INNER JOIN size ON product.size = size.id INNER JOIN category ON product.category = category.id;

SELECT address.*, address  FROM product INNER JOIN size ON product.size = size.id INNER JOIN category ON product.category = category.id;

ALTER TABLE address ADD id_user text;